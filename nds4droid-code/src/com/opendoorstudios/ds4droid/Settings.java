...
public class Settings extends PreferenceActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		final Preference cpuModePref = findPreference(CPU_MODE);
		if(cpuModePref != null) {
			if(cpuModePref instanceof ListPreference) {
				final ListPreference list = (ListPreference) findPreference(CPU_MODE);
				if(list.getEntries() != null) {
					if(list.getEntries().length >= 4)
						list.getEntries()[3] = DeSmuME.getCPUFamily() == 2 ? "DeSmuME JIT" : "exophase JIT";
				}
			}
		}
